...
const AdminSummary = () => {
  ...
  useEffect(() => {
    ...
    const fetchDepartmentSalaryData = async () => {
      try {
        const query = `year=${selectedYear}` + (selectedMonth ? `&month=${selectedMonth}` : '');
        const res = await axios.get(`http://localhost:3000/api/salary/department-salary?${query}`, {
          headers: { Authorization: `Bearer ${token}` },
        });
        if (res.data.success) {
  ...
  return (
    <div className="p-6 bg-gray-100 min-h-screen">
      ...
      {/* Overview Section */}
      <section>
        <h3 className="text-xl font-semibold text-gray-700 mb-4">Organization Overview</h3>
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-6">
          ...
          <SummaryCard icon={<FaMoneyBillWave />} text="Total Salary" number={summary.totalSalary} color="bg-indigo-600" />
        </div>
      </section>

      {/* Leave Summary Section */}
      <section className="mt-12">
        <h3 className="text-xl font-semibold text-gray-700 text-center mb-6">Leave Summary</h3>
      ...
      <section className="mt-12">
  <h3 className="text-xl font-semibold text-gray-700 text-center mb-4">Department-wise Salary Graph</h3>
  ...
  {deptSalaryData.length > 0 ? (
    ...
    <p className="text-center text-gray-500 text-sm">No salary data found for the selected period.</p>
  )}
</section>
